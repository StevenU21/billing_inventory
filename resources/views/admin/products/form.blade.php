    ...
    <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <x-inputs.select 
                ...
                label="Impuesto" 
                :options="$taxes"
                :selected="old('tax_id', $product->tax_id ?? '')"
                required
            />
            ...
            <x-inputs.select 
                ...
                :options="$units"
                :selected="old('unit_measure_id', $product->unit_measure_id ?? '')"
                required
            />
    ...
    <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm">
        ...
        <div class="grid grid-cols-1 gap-4">
             @if($product->image_url)
                <div class="mb-2">
                    <img src="{{ $product->image_url }}" alt="Imagen actual" class="w-32 h-32 object-cover rounded-md border">
                </div>
            @endif
            <x-inputs.file
                name="image"
    ...
    >
        ...
        <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm">
            ...
            <p class="text-sm text-gray-500 mb-4 px-1" x-show="attributes.length === 0">
                Agrega atributos (ej. Talla, Color) para generar variantes. Si no agregas ninguno, se creará un producto simple.
            </p>
            ...
            <div class="space-y-3">
                <template x-for="(attr, index) in attributes" :key="index">
                    <div class="flex gap-2 items-end">
                        ...
                        <div class="w-1/3">
                            <label class="block text-xs font-medium text-gray-500 mb-1" x-text="'Atributo ' + (index + 1)"></label>
                            <x-inputs.select-autocomplete
                                :label="null"
                                name="attributes[]"
                                model="attributes[index]"
                                suggestions-var="availableAttributes"
                        ...
                        <div class="flex-1">
                            <label class="block text-xs font-medium text-gray-500 mb-1">Valores (separados por coma, para generar)</label>
                            <input type="text" 
                                   x-model="attributeInputValues[index]"
                        ...
                        <button type="button" @click="removeAttribute(index)" class="mb-1.5 p-2 text-red-500 hover:bg-red-50 rounded-md">
            ...
            {{-- Generator Actions --}}
            <div class="mt-6 pt-4 border-t border-gray-900/20 dark:border-gray-900/20 flex flex-wrap gap-4 items-end justify-between" x-show="attributes.length > 0">
                <div>
                     <label class="block text-xs font-medium text-gray-500 mb-1">Precio Base (Opcional)</label>
                     <input type="number" step="0.01" x-model="bulkPrice" placeholder="0.00" class="w-40 rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500 dark:bg-gray-800 dark:border-gray-700/50 dark:text-white text-sm h-[38px]">
                </div>
                <button type="button" 
        ...
        <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm">
            <div class="flex justify-between items-center mb-4">
                ...
                <button type="button" @click="addVariant()" class="text-sm bg-blue-100 text-blue-600 px-3 py-1 rounded hover:bg-blue-200">
                    + Agregar Variante
                </button>
            </div>
            ...
            <div class="space-y-4">
                <template x-for="(variant, index) in variants" :key="index">
                    <div class="border border-gray-800/50 dark:border-gray-700/30 rounded-md p-4 relative bg-gray-50 dark:bg-gray-700">
                        <button type="button" @click="removeVariant(index)" class="absolute top-2 right-2 text-red-500 hover:text-red-700" x-show="variants.length > 1">
                            <i class="fas fa-trash"></i>
                        </button>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                            ...
                            <template x-for="(attrName, attrIdx) in attributes" :key="attrIdx">
                                <div class="md:col-span-2">
                                    <label class="block text-xs font-medium text-gray-500 mb-1" x-text="attrName || 'Atributo '+(attrIdx+1)"></label>
                                    <input type="text" 
                                           {{-- Name format: variants[0][attributes][Color] --}}
                            ...
                            <div class="md:col-span-3">
                                ...
                                <input type="text" :name="`variants[${index}][sku]`" x-model="variant.sku" class="w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500 dark:bg-gray-800 dark:border-gray-700/50 dark:text-white" placeholder="Dejar vacío para auto">
                            </div>

                            {{-- Price --}}
                            <div class="md:col-span-2">
                                <label class="block text-xs font-medium text-gray-500 mb-1">Precio *</label>
                                <input type="number" step="0.01" :name="`variants[${index}][price]`" x-model="variant.price" required class="w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500 dark:bg-gray-800 dark:border-gray-700/50 dark:text-white">
                            </div>

                            {{-- Credit Price --}}
                            <div class="md:col-span-2">
                                <label class="block text-xs font-medium text-gray-500 mb-1">Precio Crédito</label>
                                <input type="number" step="0.01" :name="`variants[${index}][credit_price]`" x-model="variant.credit_price" class="w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500 dark:bg-gray-800 dark:border-gray-700/50 dark:text-white" placeholder="(Opcional)">
                            </div>

                            {{-- Currency --}}
                            <div class="md:col-span-2">
                                 <label class="block text-xs font-medium text-gray-500 mb-1">Moneda *</label>
                                 <select :name="`variants[${index}][currency]`" x-model="variant.currency" required class="w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500 dark:bg-gray-800 dark:border-gray-700/50 dark:text-white">
