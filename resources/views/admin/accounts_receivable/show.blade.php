...
@section('content')
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 mb-8">
        {{-- Breadcrumbs --}}
        <x-breadcrumb :items="[
            ['label' => 'Módulo de Cuentas por Cobrar', 'href' => '#', 'icon' => 'fa-home'],
            ['label' => 'Cuentas por Cobrar', 'href' => route('admin.accounts_receivable.index')],
            ['label' => 'Detalles'],
        ]" />
        ...
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6">
            <div>
                <div class="flex items-center gap-3">
                    ...
                    <x-badge :color="$ar->status_color" :text="$ar->status_label" />
                    @if (!empty($ar->condition_label))
                        <x-badge :color="$ar->condition_color" :text="$ar->condition_label" />
                    @endif
                </div>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                    Cliente: {{ $ar->client_label }} • Fecha Venta: {{ $ar->sale?->formatted_created_at ?? '-' }}
                </p>
            </div>
            
            <div class="flex items-center gap-3">
                <x-link href="{{ route('admin.accounts_receivable.index') }}" variant="secondary" icon="fas fa-arrow-left">
                    Volver
                </x-link>
            </div>
        </div>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-lg font-bold text-gray-800 dark:text-gray-100">
                            {{ $ar->formatted_total_amount ?? $ar->sale?->formatted_total }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xl font-bold {{ $ar->balance_text_class }}">
            ...
            <div class="lg:col-span-2 space-y-6">
                <div x-data="{ showPanel: false }">
                    <x-table>
                        <x-slot:header>
                            <x-table.header 
                                title="Detalles de la Venta (Origen)" 
                                icon="fa-shopping-cart"
                                :collapsible="true"
                                collapsibleLabel="Ver Info de la Cuenta"
                        ...
                        <x-slot:info>
                            <x-table.info-panel :cols="4">
                                <x-table.info-item label="Cliente:" icon="fa-user" :value="$ar->client_label" />
                                <x-table.info-item label="Plazo/Condición:" icon="fa-money-check-alt" :value="$ar->payment_info_value" />
                                <x-table.info-item label="Usuario Venta:" icon="fa-user-circle" :value="$ar->sale?->user?->short_name ?? '-'" />
                                <x-table.info-item label="Fecha Venta:" icon="fa-calendar-alt" :value="$ar->sale?->formatted_created_at ?? '-'" />
                            </x-table.info-panel>
                        </x-slot:info>
                        ...
                        <x-slot:tbody>
                            @forelse($ar->sale?->saleDetails ?? [] as $detail)
                                <x-table.tr>
                                    <x-table.td>
                    ...
                    @if($ar->payments->isNotEmpty())
                        <div class="space-y-4">
                            @foreach($ar->payments as $payment)
                                <div class="flex items-start justify-between pb-3 border-b dark:border-gray-700 last:border-0 last:pb-0">
                                    <div>
