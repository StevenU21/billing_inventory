@props(['variant' => 'action', 'icon' => null, 'title' => '', 'disabled' => false, 'can' => null, 'canModel' => null, 'href' => null])
...
@if($showButton)
    ...
    @else
        <button type="button" title="{{ $title }}" {{ $attributes->merge(['class' => $classes]) }} @if($disabled) disabled
        @endif>
            @if($icon)
                <i class="{{ $icon }}"></i>
            @endif
